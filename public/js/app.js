...
function translateText(event, form) {
    ...
    if (choice == false) {
      if (validateFile(form.userPhoto.files[0])) {
        ...
        url = '/api/photo';
        executePost(postData, url);
      } else {
        $('#translatedText').html('The file you selected was invalid. Please go back and try again.');
      } //End if
    } else {
      if (validateText($("#inputText").val())) {
...
function executePost(dataObject, url) {
    $.ajax({
      url: url,
      data: dataObject,
      type: 'POST',
      ...
      success: function(responseData, status) {
          $('#translatedText').html(responseData);
          console.log(status);
        } //End success
    }); //End post
  } //End executePost
